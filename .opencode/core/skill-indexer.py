...
def _compile_frontmatter_pattern():
    """Cache compiled regex pattern for frontmatter parsing"""
    return re.compile(r"\n---\n")


@lru_cache(maxsize=128)
def _parse_tags_cached(tags_str: str) -> List[str]:
    """Cache tag parsing results"""
...
class SkillIndexer:
    ...
    def _parse_manifest_cached(self, content: str, skill_dir: str) -> Optional[Dict]:
        """
        Parse skill manifest (cached version for pure function)
        ...
        """
        try:
            pattern = _compile_frontmatter_pattern()
            end_match = pattern.search(content)
            ...
            frontmatter_lines = list(
                filter(
                    ...
                    map(str.strip, frontmatter.split("\n")),
                )
            )
            ...
            skill_entry = {
                ...
                "tags": _parse_tags_cached(str(metadata.get("tag", []))),
                "deprecated": metadata.get("deprecated", "false").lower()
                in ["true", "yes", "1", "on"],
    ...
    def _parse_manifest(self, manifest_path: str, skill_dir: str) -> Optional[Dict]:
        ...
        try:
            with open(manifest_path, "r", encoding="utf-8") as f:
                content = f.read()
            return self._parse_manifest_cached(content, skill_dir)
    ...
    def _parse_tags(self, tags) -> List[str]:
        ...
        return _parse_tags_cached(str(tags))
    ...
    def _score_skill(self, skill: Dict, query_lower: str) -> int:
        """Calculate relevance score for a single skill (pure function)"""
        score = 0
    ...
    def search_skills(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        scored_skills = (
            (skill, self._score_skill(skill, query_lower))
            for skill in self.index["skills"]
        )

        filtered = filter(lambda x: x[1] > 0, scored_skills)
        sorted_skills = sorted(filtered, key=lambda x: x[1], reverse=True)

        return [skill for skill, score in itertools.islice(sorted_skills, limit)]
    ...
    def get_index_stats(self) -> Dict:
        ...
        categories = self.index["categories"].keys()
        tags = self.index["tags"].keys()

        deprecated_count = sum(
            1 for skill in self.index["skills"] if skill.get("deprecated", False)
        )

        category_counts = {
            cat: len(skills_list) if isinstance(skills_list, list) else 1
            for cat, skills_list in self.index["categories"].items()
        }

        tag_counts = {
            tag: len(skills_list) if isinstance(skills_list, list) else 1
        ...
        if stats.get("categories"):
            ...
            for cat, skills_list in stats["categories"].items():
                ...
                if isinstance(skills_list, list):
                    ...
                    names = [
                        s["name"] if isinstance(s, dict) else s for s in skills_list
                    ]
                    if names:
                        print(
                            f"  - {cat}: {count} skills ({', '.join(names[:3])}...)"
                            if count > 3
                            else f"  - {cat}: {count} skills ({', '.join(names)})"
                        )
                    else:
        ...
        if stats.get("tags"):
            ...
            for tag, skills_list in stats["tags"].items():
                ...
                if isinstance(skills_list, list):
                    ...
                    if names:
                        print(
                            f"  - {tag}: {count} skills ({', '.join(names[:3])}...)"
                            if count > 3
                            else f"  - {tag}: {count} skills ({', '.join(names)})"
                        )
                    else:
