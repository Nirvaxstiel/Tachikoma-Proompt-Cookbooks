...
class SkillVersionManager:
    ...
    def get_skill_compatibility(
        self, skill_name: str, current_version: str
    ) -> Dict[str, any]:
        """
    ...
    def check_deprecation_impact(self, skill_name: str) -> Dict[str, any]:
        ...
        return {
            ...
            "recommendation": "or provide migration guide"
            if len(dependents) > 0
            else "No dependents",
            "version": version,
        }
    ...
    def create_version_release_notes(
        self, skill_name: str, old_version: str, new_version: str, changes: List[str]
    ) -> str:
        """
...
"""
        ...
        release_notes += (
            f"- This version maintains backward compatibility with v{old_version}\n"
        )
        release_notes += (
            f"- Follow the documentation for upgrade instructions\n\n\n## Features\n\n"
        )
    ...
    def validate_manifest(self, skill_name: str) -> Dict[str, any]:
        ...
        if not os.path.exists(manifest_path):
            return {"valid": False, "errors": ["Manifest not found"], "warnings": []}
        ...
        if "version" in metadata:
            ...
            if not re.match(r"^\d+\.\d+\.\d+$", version):
                ...
                if len(parts) != 3:
                    warnings.append(
                        f"Version {version} should be MAJOR.MINOR.PATCH format"
                    )
        ...
        if "deprecated" in metadata and metadata["deprecated"] not in [
            False,
            "false",
            "0",
            "no",
...
if __name__ == "__main__":
    ...
    check_deprecated_parser = subparsers.add_parser(
        "check", help="Check for deprecated skills"
    )
    check_deprecated_parser.add_argument(
        "--fix", action="store_true", help="Attempt to fix deprecation warnings"
    )
    ...
    if args.command == "list":
        ...
        skills = manager.get_all_skills()
        deprecated_skills = [skill["name"] for skill in skills if skill["deprecated"]]
        ...
        dirs = [
            d
            for d in os.listdir(".opencode/skills")
            if os.path.isdir(".opencode/skills/" + d)
        ]
