...
const CONFIG_FILE_PATHS = [".env", ".llmrc", ".openai/config", "~/.anthropic/config"];
...
function loadModelFormats(): Record<string, EditFormat> {
  ...
  try {
    const data = loadYaml(MODEL_FORMATS_CONFIG) as Record<string, unknown> | null;
    if (!data) {
      cachedModelFormats = {};
      return cachedModelFormats;
    }
...
function getModelFromDb(): string | null {
  for (const dbPath of OPENCODE_DB_PATHS) {
    ...
    try {
      ...
      const row = db.query("SELECT data FROM message ORDER BY time_created DESC LIMIT 1").get();
      db.close();
...
const FALLBACK_CHAINS: Record<EditFormat, EditFormat[]> = {
  ...
  [EditFormat.STR_REPLACE_FUZZY]: [EditFormat.STR_REPLACE, EditFormat.HASHLINE, EditFormat.APPLY_PATCH],
...
function getFormatDescription(formatType: EditFormat): FormatDescription {
  return FORMAT_DESCRIPTIONS[formatType] || { name: formatType, best_for: "Unknown", description: "" };
}
...
async function executeWithRetry(
  ...
  const { format: primaryFormat, confidence, reason } = selectFormat(actualModel);
  ...
  return {
    ...
    error: "Edit execution not implemented in CLI - use opencode native edit tool",
  };
}
...
function printUsage(): void {
  ...
  console.log(`  ${colors.green}detect${colors.reset}                    Auto-detect current model`);
  console.log(`  ${colors.green}recommend${colors.reset} [--model <id>]    Get format recommendation`);
  console.log(`  ${colors.green}add${colors.reset} <model> <format>        Add model format mapping`);
  console.log();
  ...
  for (const fmt of Object.values(EditFormat)) {
    ...
    if (desc) {
      console.log(`  ${colors.cyan}${fmt.padEnd(18)}${colors.reset} ${desc.description} (${desc.best_for})`);
    }
  }
  console.log();
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'recommend': {
      ...
      console.log();
      console.log(`${colors.cyan}=== Format Recommendation for ${actualModel} ===${colors.reset}`);
      console.log(`${colors.yellow}Format:${colors.reset}     ${format}`);
      console.log(`${colors.yellow}Confidence:${colors.reset} ${(confidence * 100).toFixed(0)}%`);
      ...
      if (fallbacks.length > 0) {
        console.log();
        console.log(`${colors.yellow}Fallback:${colors.reset} ${fallbacks.join(' -> ')}`);
      }
      break;
    }
    ...
    case 'add':
      if (args.length < 3) {
        console.error(`${colors.red}Error: add requires <model> <format>${colors.reset}`);
        process.exit(1);
      }
      ...
      if (!Object.values(EditFormat).includes(addFormat)) {
        console.error(`${colors.red}Error: Invalid format. Valid formats: ${Object.values(EditFormat).join(', ')}${colors.reset}`);
        process.exit(1);
      }
      
      saveModelFormat(addModel, addFormat);
      console.log(`${colors.green}Added:${colors.reset} ${addModel} -> ${addFormat}`);
      break;
