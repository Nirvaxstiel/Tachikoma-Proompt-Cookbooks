...
import { HANDOFF_TEMPLATE, fillTemplate, getTimestamp, getIsoTimestamp } from './lib/templates';
...
function createHandoff(reason: string): number {
  ...
  const accomplishments = lastActivityMatch ? `- ${lastActivityMatch[1]}` : '- No activity logged';
  ...
  const nextAction = nextActionMatch ? nextActionMatch[1] : 'Review STATE.md for current position';
  ...
  const values = {
    ...
    accomplishments,
    inProgress,
    decisions: decisionRows.length > 0 ? decisionRows.join('\n') : 'No decisions',
    decisionsTable: decisionRows.length > 0 ? decisionRows.join('\n') : '| *No decisions yet* | - | - |',
    deferredTable: deferredRows.length > 0 ? deferredRows.join('\n') : '| *No deferred issues* | - | - | - |',
    blockersTable: blockerRows.length > 0 ? blockerRows.join('\n') : '| *No active blockers* | - | - |',
    nextAction,
  ...
  console.log();
  console.log(`Reason: ${colors.green}${reason || 'Session break'}${colors.reset}`);
...
function resumeFromHandoff(handoffFile?: string): number {
  ...
  if (handoffFile) {
    handoffPath = handoffFile.includes('/') || handoffFile.includes('\\')
      ? handoffFile
  ...
  if (nextActionMatch) {
    console.log(colors.green + nextActionMatch[1].split('\n')[0] + colors.reset);
  } else {
