...
class VectorStore:
    ...
    def search(
        self, query: str, limit: int = 5, filters: Dict[str, Any] = None
    ) -> List[Dict]:
        ...
        for i, vec in enumerate(self.vectors):
            score = self._dot(query_embedding, vec) / (
                self._norm(query_embedding) * self._norm(vec) + 1e-8
            )
...
class PropertyGraph:
    ...
    def get_or_create_node(
        self, name: str, label: str = "Entity", properties: Dict = None
    ) -> str:
    ...
    def create_relationship(
        self, source_id: str, rel_type: str, target_id: str, properties: Dict = None
    ) -> str:
        ...
        for result in base_results:
            ...
            if valid_from <= query_time:
                if (
                    ...
                    results.append(
                        {**result, "valid_from": valid_from, "valid_until": valid_until}
                    )

        return results

    def query_time_range(
        self, query: Dict, start_time: datetime, end_time: datetime
    ) -> List[Dict]:
